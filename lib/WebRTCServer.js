...
function WebRTCServer(dependencies, adapter, webrtcConfig) {
  ...
  var onAuthenticate = dependencies('connector').lib.auth;

  self.events.on('log', function(level, logText, logFields, next) {
    self.logger.log(level, 'WebRTC: %s', logText, logFields ? logFields : '');
    if (next && typeof next === 'function') {
      next();
    }
  });

  self.events.on('authenticate', onAuthenticate);

  self.events.on('iceconfig', function(connectionObj, callback) {
    self.logger.debug('WebRTC Requesting ICE configuration ');
    var myIceselfs = webrtcConfig.appIceselfs || [];
    callback(null, myIceselfs);
  });
